...
export default {
  ...
  data: () => ({
    statusMap: {
      online: { icon: "mdi-cloud-check", color: "green" },
    ...
    nowTimestampFirstPeriodMs: 100,
  }),
  computed: {
    lastConnectionFormated() {
      return strftime(
        "%Y-%m-%d %H:%M:%S",
        new Date(this.lastConnection * 1000)
      );
    },
    lastConnectionElapsedHours() {
      ...
      return (
        (Math.round(this.nowTimestamp / 1000) - this.lastConnection) / 3600
      );
    },
    connectionStatus() {
  ...
  created() {
    setTimeout(this.refreshNowTimestamp, this.nowTimestampFirstPeriodMs);
    this.nowTimestampTimer = setInterval(
      this.refreshNowTimestamp,
      this.nowTimestampRefreshPeriodMs
    );
  },
  destroyed() {
