<template>
  <v-container>
    <v-card>
      <v-card-title>Devices
        <v-spacer></v-spacer>
        <v-btn @click="showNewDeviceDialog = true">Add Device</v-btn>
        <NewDeviceDialog :show="showNewDeviceDialog" @closed="showNewDeviceDialog = false" />
      </v-card-title>
    </v-card>
    <v-container class="px-0">
      <v-scroll-y-transition group>
        <DeviceCard :data="device" v-for="(device, index) in devices" :key="index" />
